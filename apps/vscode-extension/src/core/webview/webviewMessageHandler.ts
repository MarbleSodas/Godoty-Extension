...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		...
		case "openGlobalKeybindings":
			vscode.commands.executeCommand("workbench.action.openGlobalKeybindings", message.text ?? "godoty.")
			break
		...
		case "ghostServiceSettings":
			...
			await provider.postStateToWebview()
			vscode.commands.executeCommand("godoty.ghost.reload")
			break
		...
		case "saveApiConfiguration":
			if (message.text && message.apiConfiguration) {
				try {
					...
					await updateGlobalState("listApiConfigMeta", listApiConfig)
					vscode.commands.executeCommand("godoty.ghost.reload") // kilocode_change: Reload ghost model when API provider settings change
				} catch (error) {
					provider.log(
