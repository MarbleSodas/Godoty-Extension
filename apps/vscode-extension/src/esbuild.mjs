...
async function main() {
	...
	// godoty_change start: Bundle the Godoty MCP server
	/**
	 * @type {import('esbuild').BuildOptions}
	 */
	const mcpServerConfig = {
		...
		sourcesContent: false,
		platform: "node",
		entryPoints: [path.join(__dirname, "../../../packages/godoty-mcp/src/index.ts")],
		outfile: "dist/mcp/godoty-mcp.js",
		// Use the extension's node_modules for resolving dependencies
		nodePaths: [path.join(__dirname, "node_modules")],
	}
	// godoty_change end

	const [extensionCtx, workerCtx, mcpCtx] = await Promise.all([
	...
	if (watch) {
		await Promise.all([extensionCtx.watch(), workerCtx.watch(), mcpCtx.watch()])
		copyLocales(srcDir, distDir)
		setupLocaleWatcher(srcDir, distDir)
	} else {
		await Promise.all([extensionCtx.rebuild(), workerCtx.rebuild(), mcpCtx.rebuild()])
		await Promise.all([extensionCtx.dispose(), workerCtx.dispose(), mcpCtx.dispose()])
	}
}
