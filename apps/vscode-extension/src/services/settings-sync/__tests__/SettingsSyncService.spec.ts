	...
	describe("initialize", () => {
		it("should register sync keys when settings sync is enabled", async () => {
			...
			expect(mockGlobalState.setKeysForSync).toHaveBeenCalledWith([
				"godoty.allowedCommands",
				"godoty.deniedCommands",
				"godoty.autoApprovalEnabled",
				"godoty.fuzzyMatchThreshold",
				"godoty.diffEnabled",
				"godoty.directoryContextAddedContext",
				"godoty.language",
				"godoty.customModes",
				"godoty.firstInstallCompleted",
				"godoty.telemetrySetting",
			])
		...
		it("should use default value true when setting is not configured", async () => {
			...
			expect(mockGlobalState.setKeysForSync).toHaveBeenCalledWith(
				expect.arrayContaining(["godoty.allowedCommands", "godoty.deniedCommands"]),
			)
		})
		...
		it("should work without outputChannel parameter", async () => {
			...
			expect(mockGlobalState.setKeysForSync).toHaveBeenCalledWith(
				expect.arrayContaining(["godoty.allowedCommands"]),
			)
		})
	})
