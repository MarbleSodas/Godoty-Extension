...
// godoty_change start
let godotyAuth: GodotyAuthService | undefined
let godotyMcp: GodotyMCPService | undefined
let godotStatusBar: GodotStatusBar | undefined
let userStatusBar: UserStatusBar | undefined
// godoty_change end
...
export async function activate(context: vscode.ExtensionContext) {
	...
	try {
		...
		outputChannel.appendLine(
			`[Godoty] Failed to initialize services: ${error instanceof Error ? error.message : String(error)}`,
		)
	}
	// godoty_change end
...
// godoty_change start
async function showGodotyUserMenu(): Promise<void> {
	const user = godotyAuth?.getUser()
	...
	if (user) {
		...
		const choice = await vscode.window.showQuickPick(choices, {
			placeHolder: "Godoty Account",
		})

		if (choice?.value === "signOut") {
			await godotyAuth?.signOut()
		}
	} else {
		await godotyAuth?.signIn()
	}
}

async function showGodotyConnectionMenu(): Promise<void> {
	const isConnected = godotyMcp?.isConnected ?? false

	const choices: (vscode.QuickPickItem & { value?: string })[] = [
		{
			label: isConnected ? "$(check) Connected to Godot" : "$(x) Not connected",
			description: isConnected ? "MCP server running" : "MCP server not running",
			kind: vscode.QuickPickItemKind.Separator,
		},
		{ label: "$(refresh) Restart MCP Server", value: "restart" },
		{ label: "$(output) Show MCP Logs", value: "logs" },
	]

	const choice = await vscode.window.showQuickPick(choices, {
		placeHolder: "Godot Connection",
	})

	switch (choice?.value) {
		case "restart":
			await godotyMcp?.restart()
			break
		case "logs":
			godotyMcp?.showLogs()
			break
