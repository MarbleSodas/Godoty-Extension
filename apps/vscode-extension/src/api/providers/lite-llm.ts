...
// godoty_change start
/**
 * Godoty LiteLLM proxy URL - used for detecting Godoty backend
 */
export const GODOTY_LITELLM_URL = "https://litellm-production-150c.up.railway.app"

/**
 * Check if the given URL is the Godoty LiteLLM backend
 */
export function isGodotyBackend(baseUrl: string | undefined): boolean {
	if (!baseUrl) return false
	// Normalize URL for comparison (remove trailing slash)
	const normalizedUrl = baseUrl.replace(/\/$/, "")
	const normalizedGodotyUrl = GODOTY_LITELLM_URL.replace(/\/$/, "")
	return normalizedUrl === normalizedGodotyUrl
}
// godoty_change end
 ...
 * 
 * godoty_change: When connected to Godoty's LiteLLM proxy, it uses Supabase-provided
 * API keys (godotyApiKey) instead of manual litellmApiKey configuration.
 */
export class LiteLLMHandler extends RouterProvider implements SingleCompletionHandler {
	// godoty_change start
	private readonly _isGodotyBackend: boolean
	// godoty_change end

	constructor(options: ApiHandlerOptions) {
		// godoty_change start: Detect Godoty backend and use appropriate API key
		const baseUrl = options.litellmBaseUrl || "http://localhost:4000"
		const isGodoty = isGodotyBackend(options.litellmBaseUrl)
		
		// For Godoty backend, prefer godotyApiKey (from Supabase auth) over litellmApiKey
		const apiKey = isGodoty 
			? (options.godotyApiKey || options.litellmApiKey || "dummy-key")
			: (options.litellmApiKey || "dummy-key")
		// godoty_change end
		...
		// godoty_change start
		this._isGodotyBackend = isGodoty
		// godoty_change end
	}

	// godoty_change start
	/**
	 * Returns true if this handler is connected to Godoty's LiteLLM backend
	 */
	get isGodotyBackend(): boolean {
		return this._isGodotyBackend
	}

	/**
	 * Override fetchModel to provide better error messages for Godoty users
	 */
	override async fetchModel() {
		if (this._isGodotyBackend && !this.options.godotyApiKey && !this.options.litellmApiKey) {
			throw new Error(
				"Please sign in to Godoty to use this provider. " +
				"Click the user icon in the status bar or run 'Godoty: Sign In' command."
			)
		}
